...
from processing import to_ndarray, convert_binary_labels_to_minus_plus
...
def run_random_forest_pipeline(X_train, X_test, y_train, y_test, target_names=None):
...
def run_adaboost_pipeline(X_train, X_test, y_train, y_test, target_names):
    _create_log("Starting AdaBoost training...", "info", "adaboost_log.log")
    y_train_boost = convert_binary_labels_to_minus_plus(y_train)
    y_test_boost = convert_binary_labels_to_minus_plus(y_test)
    model = AdaBoost(n_estimators=config.N_ESTIMATORS_ADABOOST)
    _create_log("Fitting AdaBoode model...", "info", "adaboost_log.log")
